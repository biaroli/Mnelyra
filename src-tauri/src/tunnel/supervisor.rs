...
impl TunnelSupervisor {
    ...
    /// Replace an active Cloudflare connector without tearing the working one
    /// down first. The candidate must complete normal startup validation and,
    /// for Named Tunnel, emit `Registered tunnel connection`. Only then is it
    /// published as the supervised session and the previous connector retired.
    pub async fn restart_cloudflare_rolling(
        &mut self,
        profile: &WorkspaceProfile,
        kind: TunnelServiceKind,
        settings: &AppSettings,
        ...
        if tunnel_type_for(profile, kind) != "cloudflare" {
            return self.start(profile, kind, settings).await;
        }

        // A route currently owned by frpc needs the existing FRP route-removal
        // machinery. The zero-downtime path below is specifically Cloudflare
        // connector -> Cloudflare connector (Quick <-> Named included).
        ...
        // Everything that can reject the replacement happens while the current
        // connector is still owned by `self.sessions` and remains untouched.
        ...
        // process so requests can immediately move to the replacement.
        ...
        if let Some(mut previous) = previous {
            if let Some(child) = previous.child.take() {
                let _ = cloudflare::stop_child(child, previous.pid).await;
            } else if let Some(pid) = previous.pid {
...
fn validate_tunnel_requirements(
    ...
    if mode == "named" {
        ...
        let named_url = named_url.trim().trim_end_matches('/');
        if named_url.is_empty() {
            return Err(AppError::Message(
                "Cloudflare 命名隧道模式需要填写固定公网地址。".into(),
            ));
        }
        if !named_url.starts_with("https://") {
            return Err(AppError::Message(
...
fn cloudflare_config(
    ...
    match kind {
        TunnelServiceKind::Mcp => {
            let token = SecretStore::get_shared("cloudflare_token")?.unwrap_or_default();
            Ok((
