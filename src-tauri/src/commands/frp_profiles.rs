...
#[tauri::command]
pub async fn set_last_workspace(state: State<'_, AppState>, id: String) -> AppResult<()> {
    let should_activate = state.with_settings(|store| {
        ...
        let mut settings = store.settings();
        settings.last_workspace_id = id.clone();
        let configured = settings.general.configured;
        store.update_settings(settings)?;
        Ok(configured)
    })?;

    if should_activate {
        crate::commands::runtime::activate_workspace_mcp(&state, &id, false).await?;
    }
