...
pub async fn restart_tunnel(
    ...
    let result = {
        ...
        } else if was_running && tunnel_type == "cloudflare" {
            // Keep the current Quick/Named connector online until the candidate
            // has really connected. This is the critical Quick -> Named handover.
            guard
                .restart_cloudflare_rolling(&profile, kind, &settings)
                .await
    ...
    let result = {
        ...
        if was_tunnel_running && tunnel_type_for(&profile, kind) == "frp" {
            guard
                ...
                .await
                .map_err(|error| (error, guard.route_profile(&id, kind)))
        } else if was_tunnel_running && tunnel_type_for(&profile, kind) == "cloudflare" {
            guard
                .restart_cloudflare_rolling(&profile, kind, &settings)
                .await
                .map_err(|error| (error, None))
