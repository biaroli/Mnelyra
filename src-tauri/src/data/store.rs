...
impl DataStore {
    pub fn load() -> AppResult<Self> {
        ...
        let migrated_global = if !data.general.configured {
            let source = data
                .profiles
                .iter()
                .find(|profile| profile.id == data.last_workspace_id)
                .or_else(|| data.profiles.first())
                .cloned();
            if let Some(profile) = source {
                if !data.shared_secrets.contains_key("oauth_client_id")
                    && !profile.auth.oauth_client_id.trim().is_empty()
                {
                ...
                data.auth = crate::settings::GlobalAuthConfig::from_profile(&profile);
                true
            } else {
                false
            }
        ...
        let migrated_runtime_v2 = if data
            .app_secrets
            .get("migration")
            .and_then(|items| items.get("global_runtime_v2"))
            .is_none()
        {
            let source = data
                .profiles
            ...
            if let Some(profile) = source {
                // The workspace copy is the last known-good source during the
                // transition. Move service/tunnel/policy state to the one global
                // runtime configuration, but deliberately keep the already-stable
                // global OAuth identity/secrets unchanged.
                data.general = crate::settings::GlobalGeneralConfig::from_profile(&profile);
                ...
                data.app_secrets
                    .entry("migration".to_string())
                    .or_default()
                    .insert("global_runtime_v2".to_string(), "1".to_string());
                true
            } else {
                false
            }
