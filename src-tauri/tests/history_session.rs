...
use mnelyra_lib::tools::{list_tools_for_profile, ToolContext};
...
fn bootstrap_reuses_legacy_rootrelay_history_directory_in_place() {
    ...
    let first = invoke_ok(
        ...
        json!({"session_key": "legacy-migration", "initial_user_input": "保留旧历史"}),
    );
    assert_eq!(first["current_path"], ".mnelyra/history-session/1.md");

    fs::rename(
        workspace.path().join(".mnelyra"),
        workspace.path().join(".rootrelay"),
    )
    .expect("stage legacy history directory");
    assert!(!workspace.path().join(".mnelyra").exists());
...
fn bootstrap_prefers_host_session_and_preserves_initial_input() {
    ...
    assert_eq!(boot["current_path"], ".mnelyra/history-session/1.md");
    ...
    let archive = fs::read_to_string(workspace.path().join(".mnelyra/history-session/1.md"))
        .expect("read archive");
    assert!(archive.contains(first_request));
    assert!(!archive.contains("继承的历史摘要"));
    assert!(workspace
        .path()
        .join(".mnelyra/history-session/memory/state.json")
        .is_file());
    assert!(workspace
        .path()
        .join(".mnelyra/history-session/memory/manifest.json")
        .is_file());
}
...
fn search_and_read_return_precise_lossless_archive_pages() {
    ...
    assert_eq!(hit["path"], ".mnelyra/history-session/2.md");
    ...
    let original = fs::read_to_string(workspace.path().join(".mnelyra/history-session/2.md"))
    ...
    assert_eq!(
        assert_err(&invoke(
            &ctx,
            "history_session_read",
            json!({"path": ".mnelyra/history-session/README.md"}),
        ))["error"]["code"],
...
fn read_without_max_bytes_uses_bounded_lossless_pages() {
    let (workspace, _harness, ctx) = test_context();
    let dir = workspace.path().join(".mnelyra/history-session");
...
fn rebuilds_derived_files_without_changing_existing_archives() {
    ...
    let memory = workspace.path().join(".mnelyra/history-session/memory");
...
fn real_history_fixture_preserves_archives_when_opted_in() {
    ...
    let (workspace, _harness, ctx) = test_context();
    let target = workspace.path().join(".mnelyra/history-session");
...
fn prepare_history(root: &std::path::Path) {
    let dir = root.join(".mnelyra/history-session");
